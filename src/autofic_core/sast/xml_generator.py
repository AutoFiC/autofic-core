...
class RenderOptions:
    ...
    Attributes:
        tool_name: 생성 메타에 기록할 도구 이름.
        schema_location: XML 루트 schemaLocation 속성에 주입될 경로 문자열.
        include_env: 각 취약점에 <ENV> 블록 포함 여부.
        include_tracking: <TRACKING> (이슈/커밋 placeholder) 추가 여부.
        include_mitigations: <MITIGATION> 기본 요약 추가 여부.
        context_lines_before: <CONTEXT before="n"> 로 주변 코드 라인 수(앞).
        context_lines_after: <CONTEXT after="n"> 로 주변 코드 라인 수(뒤).

    Forward Extension:
        스키마에 xsd:any 확장 슬롯이 추가되었으므로 새로운 요소(예: CONSTRAINTS, KB, RISK_MODEL 등)를
        추가하려면:
          1. BaseSnippet 또는 별도 매핑에서 데이터 확보
          2. 아래 generate loop 내부에서 조건부 ET.SubElement 로 추가
          3. 프롬프트/PR 생성 로직이 필요한 경우 해당 태그 파싱 로직 확장
        태그를 추가하더라도 기존 스키마 검증은 processContents="lax" 로 인해 깨지지 않습니다.
    """
...
def generate_custom_context(
    ...
    NOTE(Forward-Extension): 스키마에 xsd:any 슬롯이 있으므로 새 태그를 추가해도 검증 실패하지 않습니다.
    예: v_el = ET.SubElement(v, f"{{{XML_NS}}}CONSTRAINTS"); 이후 하위 ITEM 들 추가 가능.
    """
